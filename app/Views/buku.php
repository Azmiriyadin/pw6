    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="reset_update_data()">Batal</button>
            </div>
        </div>
    </div>
</div>
<script>
    ...
    function tambah_data(){
        ...
        if(kode == "" || judul == "" || isbn == "" || pengarang == "" || penerbit == "" || tahun == "" || rak == "" ){
            swal({title: "Gagal", text: "Ada Isisan Yang Masih Kosong", icon: "error"});
            return;
        }
        $.ajax({
            url: "<?= BASEURLKU; ?>addbook",
            method: "POST",
            data: {kodex: kode, judulx: judul, isbnx: isbn, pengarangx: pengarang, penerbitx: penerbit, tahunx: tahun, rakx: rak},
            cache: "false",
            success: function(response){
                let data = JSON.parse(response);
                if(data.kode == "1"){
                    swal({title: "Berhasil", text: data.pesan, icon: "success"});
                    reset_tambah_data();
                    tabel.ajax.reload();
                }else{
    ...
    function hapus_data(){
        let kode = $("#txtkodee").val();
        if(kode == ""){
            swal({title: "Gagal", text: "Kode Buku Masih Kosong", icon: "error"});
            return;
        }
        swal({
            title: 'konfirmasi',
