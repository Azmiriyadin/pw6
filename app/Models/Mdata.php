...
class Mdata extends Model
{
    ...
    {
        $sql = "SELECT Tahun_Terbit, COUNT(*) AS Jumlah FROM buku GROUP BY Tahun_Terbit ORDER BY COUNT(*)DESC
                LIMIT 1";
        $dt = db_connect()->query($sql);
        if ($dt) {
            return $dt->getRow();
    ...
    {
        $sql = "SELECT Penerbit, COUNT(*) AS Jumlah FROM buku_view GROUP BY Penerbit ORDER BY COUNT(*)DESC
                LIMIT 1";
        $dt = db_connect()->query($sql);
        if ($dt) {
            return $dt->getRow();
    ...
    {
        $sql = "SELECT SUBSTR(Rak, 1, 1) AS Rak, COUNT(*) AS Jumlah FROM buku GROUP BY SUBSTR(Rak, 1, 1)
